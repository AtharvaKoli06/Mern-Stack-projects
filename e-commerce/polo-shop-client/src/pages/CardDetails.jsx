...
import { productDetailsWithId } from "../redux/slices/DataWithId.slice";

const CardDetails = () => {
  const { id } = useParams();
  const dispatch = useDispatch();

  const productDataWithId = useSelector((state) => state.cardDetailsWithId);

  useEffect(() => {
    dispatch(productDetailsWithId(id));
  }, [dispatch]);

  const { loading, error, data } = productDataWithId;
          ...
          <div className="w-full flex gap-3 items-start justify-center p-2">
            <div className="grid lg:grid-cols-2 gap-2 w-3/5">
              {singleProduct.img.map((item) => (
                <div
                  key={item._id}
                  className="m-auto w-full p-2 cursor-pointer"
                >
